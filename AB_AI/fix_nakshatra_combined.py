...
for file in files:
    ...
    # 4. Universal Hero Fix (Mobile) - if it applies here? Nakshatra was fixed manually.

    # 5. Nakshatra Hero Overhaul (COMBINED)
    combined_css = """<style>
      .cvd-hero {
        background: #FFFFFF;
        position: relative;
      ...
      /* Mobile Overrides (MUST FOLLOW DESKTOP) */
      @media (max-width: 768px) {
        ...
        /* Apply identical robust cropping as our universal fix */
        .cvd-hero-img-wrap img { width: 100%; max-width: 400px; height: 300px; margin: 0 auto; object-fit: cover; object-position: 85% center; display: block; border-radius: 50% !important; margin-right: 0; }
      }
</style>"""
    ...
    if 'cvd-hero-banner' in content:
        ...
        # INJECT THE COMBINED CSS PERFECTLY BEFORE LAYOUT CONTAINER
        content = content.replace('    <!-- START MAIN LAYOUT WITH SIDEBAR -->', combined_css + '\n    <!-- START MAIN LAYOUT WITH SIDEBAR -->')
        
        # We don't touch @media (max-width: 768px) in <head> anymore since mobile css is embedded in `combined_css`.
...
print("Safely fixed Nakshatra CSS cascading overflow.")
