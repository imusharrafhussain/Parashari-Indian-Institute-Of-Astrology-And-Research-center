...
print(f"Processing {len(html_files)} HTML files and {len(js_files)} JS files...")

# Squeeze Fix & Hero Center Fix function
def fix_css(content):
    # 1. Squeeze Fix
    ...
    # 2. Before/After Fix
    # Replace any messy BA block inside @media (max-width: 768px) safely
    # We find .cvd-ba-container matched block and replace it up to .cvd-learn-grid
    if '.cvd-ba-container {' in content and '.cvd-learn-grid' in content:
        # Simple string replacement for known bad blocks
        content = re.sub(
    ...
    # 4. Universal Hero Fix
    # Replace only the SECOND occurrence or within the CORRECT media query
    # Find .cvd-hero-img-wrap img { ... } that is close to .cvd-section { padding: 20px 15px...
    # to ensure it's the mobile query
    content = re.sub(
...
for file in html_files + js_files:
    ...
    css_fixed = fix_css(orig)
    
    # 5. Nakshatra Overhaul (Only for nakshatra HTML)
    final_content = css_fixed
    if 'nakshatra' in file.lower() and '.html' in file:
      ...
      .cvd-hero-img-wrap img {
        ...
        price_match = re.search(r'Enroll Now – <span[^>]+>₹([0-9,]+)</span> ₹([0-9,]+)', final_content)
        ...
        if 'cvd-hero-banner' in final_content:
            final_content = re.sub(r'\.cvd-hero-banner \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-banner img \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-overlay \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-title \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-subtitle \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-below-hero \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-below-hero-heading \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-below-hero-desc \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-badge \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-btn \{.+?\}', '', final_content, flags=re.DOTALL)
            final_content = re.sub(r'\.cvd-hero-btn:hover \{.+?\}', '', final_content, flags=re.DOTALL)

            final_content = final_content.replace('    <div class="cvd-layout-container"', std_desktop_css + '\n    <div class="cvd-layout-container"')
            idx = final_content.rfind('@media (max-width: 768px) {')
            if idx != -1:
                insert_pos = final_content.find('{', idx) + 1
                final_content = final_content[:insert_pos] + '\n' + std_mobile_css + final_content[insert_pos:]
            
            final_content = re.sub(r'<!-- HERO: Image banner.+?One-time Payment - Lifetime Access</span>\s*</div>\s*</section>', new_hero_html, final_content, flags=re.DOTALL)
